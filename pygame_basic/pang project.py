...
image_path = os.path.join(current_path, ) #pygame_basic 폴더 위치 반환

# 배경 
background = pygame.image.load(os.path.join(image_path, "pang_background.png"))

# 스테이지 만들기
stage = pygame.image.load(os.path.join(image_path, "pang_stage.jpg"))
stage_size = stage.get_rect().size # 이미지의 크기 불러오기
stage_height = stage_size[1] # 스테이지의 높이 위에 캐릭터를 뒤기 위해 사용
...
ball_images = [
    pygame.image.load(os.path.join(image_path, "pang_ballon1.jpg")),
    pygame.image.load(os.path.join(image_path, "pang_ballon2.jpg")),
    pygame.image.load(os.path.join(image_path, "pang_ballon3.jpg")),
    pygame.image.load(os.path.join(image_path, "pang_ballon4.jpg"))]
...
while running:
    ...
    # 천장에 닿은 무기 없애기
    weapons = [[w[0], w[1]] for w in weapons if w[1] > 0 ] # 좌표가 0 이 되어버리면 없애 버리기
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 공과 캐릭터 충돌 체크
        if character_rect.colliderect(ball_rect): # 충돌 했는지
            print("풍선과 충돌 했어요")
            running = False
            break
        ...
        break # 안쪽 for 문에서 break를 만나면 여기서 진입 가능.2 중 for문을 한번에 탈출 가능
